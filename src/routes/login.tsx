  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background p-6">
      <div className="w-full max-w-sm rounded-3xl border border-border bg-card p-8">
        ...
        <h1 className="mt-4 font-display text-3xl text-card-foreground">{t("common.welcomeBack")}</h1>
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          <Button type="submit" className="w-full" disabled={loading}>{t("common.signIn")}</Button>
        </form>
        ...
        <p className="mt-4 text-center text-xs"><Link to="/reset-password" className="text-muted-foreground hover:text-primary">{t("common.forgotPassword")}</Link></p>
        <p className="mt-4 text-center text-sm text-muted-foreground">
          {t("common.noAccount")} <Link to="/signup" className="text-primary">{t("common.signUp")}</Link>
        </p>
      </div>
    </div>
  );
